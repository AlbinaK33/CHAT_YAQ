...
const RoomPage: React.FC = () => {
  ...
  const [liked1, setliked1] = useState(-1);
  const [liked2, setliked2] = useState(-1);
  const [liked3, setliked3] = useState(-1);
  const [liked4, setliked4] = useState(-1);
  const [liked5, setliked5] = useState(-1);
  const [liked6, setliked6] = useState(-1);
  const [liked7, setliked7] = useState(-1);
  const [liked8, setliked8] = useState(-1);
  const rooms = useMemo(
    () => [
      {
        index: 1,
  ...
  const handleNavigation=()=>{
    navigate("/roomsrecent");
  }
  const filteredRooms = useMemo(() => {
    const likedRooms = rooms.filter((room) => room.liked!==0);
    likedRooms.sort((a, b) => b.liked - a.liked);
    return [...likedRooms];
  }, [rooms]);
  ...
  return (
    <div className={"rooms-page"}>
      ...
      <main>
        <div className="recent" onClick={handleNavigation}>
          <h2>Нещодавні</h2>
          <div className="info">
